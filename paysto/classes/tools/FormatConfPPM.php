...
class FormatConfPPM
{
            ...
            foreach ($input_array as $item) {
                ...
                foreach ($properties as $property => $value) {
                    if (is_array($post_data) && array_key_exists($item[$id_key], $post_data)
                        ...
                        $return_data[$item[$id_key]][$property] = ToolsModulePPM::formatValue(
    ...
    {
        return self::prepareSettingGroup(ConfPPM::getConf('SETTING_GROUPS', ConfPPM::TYPE_ARRAY));
    }

    public static function getSettingOrderStates()
    {
        return self::prepareSettingOrderStates(ConfPPM::getConf('SETTING_ORDER_STATES', ConfPPM::TYPE_ARRAY));
    }
}
